...
//     return;
  ...
  //   if (_ir_objects[ID_SIDE_LEFT_BACK]->getDistance() < SIDE_DIST_THRESHOLD || 
  ...
  //   }
    ...
    int side_irs[2] = {_ir_objects[ID_SIDE_LEFT_FRONT]->getAvgAnalog(),
        _ir_objects[ID_SIDE_RIGHT_FRONT]->getAvgAnalog()};

    // Serial.print(side_irs[0]);
    // Serial.print(", ");
    // Serial.println(side_irs[1]);
 ...
 //    // TODO: ADD CASE WHERE BOTTLE IN FRONT OF OBSTACLE THAT IS WITHIN THRESHOLD
 	if (top_irs[0] < TOP_OBSTACLE_THRESHOLD || top_irs[1] < TOP_OBSTACLE_THRESHOLD) {
 		...
 		_motor_left += (int) 1.4*(MEAN_MOTOR_SPEED*(top_irs[1] - top_irs[0])/80.0);
 		_motor_right += (int) 1.4*(MEAN_MOTOR_SPEED*(top_irs[0] - top_irs[1])/80.0);

    } 

    if (side_irs[0] > 75 || side_irs[1] > 75) {
        _motor_left += (int) (MEAN_MOTOR_SPEED*(side_irs[0] - side_irs[1])/150.0);
        _motor_right += (int) (MEAN_MOTOR_SPEED*(side_irs[1] - side_irs[0])/150.0);
    }

        // _motor_left = (int) ((top_irs[1] - TARGET)/10.0);
        // _motor_right = (int) ((top_irs[0] - TARGET)/10.0);
 ...
 // //        _ir_objects[ID_FRONT_BOT_RIGHT_IN]->getDistance()};
    ...
    // Set motors so as to move in bottle direction
 	_motor_left += (int) ( MEAN_MOTOR_SPEED/100.0*(bottom_irs[0] - top_irs[1]));
	_motor_right += (int) ( MEAN_MOTOR_SPEED/100.0*-(bottom_irs[0] - top_irs[1]));


 //    _motor_left += (int) (MEAN_MOTOR_SPEED/100.0*(bottom_irs[1] - top_irs[0]) );
 //    _motor_right += (int) ( -MEAN_MOTOR_SPEED/100.0*(bottom_irs[1] - top_irs[0]) );
...
void Robot::avoid() {
    ...
    Serial.println(_bumper_hit);
    stopMotors();
    delay(MOTOR_STOP);
    setSpeeds(-(min(MAX_MOTOR_SPEED, _motor_left)),-(min(MAX_MOTOR_SPEED,_motor_right)));// do the reverse movement from before

    delay(MOTOR_BACK);
    
    if ((_bumper_hit == 1) | (_bumper_hit == 2)){ // front or back bumpers triggered
        // reverse / forward while turning a little bit
        setSpeeds(-(min(MAX_MOTOR_SPEED, _motor_left)),-(min(MAX_MOTOR_SPEED, _motor_right) - DELTA_AVOID));
        delay(MOTOR_TURN);
    }
    else { // side bumpers triggered
        setSpeeds((min(MAX_MOTOR_SPEED, _motor_right)),-(min(MAX_MOTOR_SPEED,_motor_left))); // reverse motor signal in order to go on in different direction.
        delay(MOTOR_TURN);
    }
