...
{		
		if (SIDE == 0){                          // Move Left///////////////////////////
			...
			Speed_H = Speed >> 8;
			Speed_L = Speed;                     // 16 bits - 2 x 8 bits variables
			Checksum = (~(ID + AX_vel_LENGTH + AX_WRITE_DATA + AX_GOAL_vel_L + Speed_L + Speed_H))&0xFF;
			
			switchCom(Direction_Pin,Tx_MODE);
			sendData(AX_START);                // Send Instructions over Serial
			sendData(AX_START);
			sendData(ID);
			sendData(AX_vel_LENGTH);
			sendData(AX_WRITE_DATA);
			sendData(AX_GOAL_vel_L);
			sendData(Speed_L);
			sendData(Speed_H);
			sendData(Checksum);
			delayus(TX_DELAY_TIME);
...
{	
    Checksum = (~(ID + AX_POS_LENGTH  + AX_READ_DATA + AX_PRESENT_vel_L + AX_BYTE_READ_POS))&0xFF;
