...
MySharpIR::MySharpIR(int irPin, int avg, int sensorModel) {
...
int MySharpIR::distance() {
    ...
    for (int i=0; i<_avg; i++){
        ...
        analog_signal += analogRead(_irPin);
        
    }

    float analog_avg = analog_signal/_avg;
 
    //for LongRangeModel
    int dist = 8508.367*pow(analog_avg, -1.0996);
